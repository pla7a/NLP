...
def sem(tr):
    ...
    # AN on LHS productions
    elif (rule == 'AN -> A AN'):
        return "(\\x.(" + sem(tr[0]) + "(x) & " + sem(tr[1]) + "(x)))"
    elif (rule == "AN -> N"):
        return sem(tr[0]) 
        
    # NP on LHS productions
    elif (rule == "NP -> P"):
        return "(\\x.(x = " + sem(tr[0]) + "))"
    elif (rule == "NP -> Nom"):
        return sem(tr[0]) 
